...
#(isolated with hierarchical clustering on normalized cyt values)
...
groups = getGroups('data/cluster_groups/ordered_rr_norm.txt', norm_stops)
...
for lista1 in liste_a:
	...
	matrix.append(pvals)
writeStat(matrix, 'norm_ther1_end')
...
for lista1 in liste_a:
	...
	for lista2 in liste_b:
		...
		pvals.append(str(stats.fisher_exact(table)[1]))
	matrix.append(pvals)
writeStat(matrix, 'norm_ther_line')