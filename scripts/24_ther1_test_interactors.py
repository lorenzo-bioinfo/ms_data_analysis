...
#(isolated with hierarchical clustering on sumscore cyt values)
...
groups = getGroups('data/cluster_groups/ordered_rr_interactors.txt', sum_stops)
...
for lista1 in liste_a:
	...
	matrix.append(pvals)
writeStat(matrix, 'int_ther1_end')
...
for lista1 in liste_a:
	...
	for lista2 in liste_b:
		...
		pvals.append(str(stats.fisher_exact(table)[1]))
	matrix.append(pvals)
writeStat(matrix, 'int_ther_line')