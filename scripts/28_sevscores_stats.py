...
#extracting groups from severity scores based clustering (ward linkage method)

sev_stops = ['589', '24', '94']

sev_groups = getGroups('./data/cluster_groups/ordered_rr_sevscores_norm.txt', sev_stops)

for gp in sev_groups:
	print(gp, len(gp), '\n\n')
'''
#Performing Kolmogorov-Smirnov test on different parameters
#Each group will be tested against all others

def cross_ks(s1, s2, s3, s4, filename):
	tup1 = (s1, s2, s3, s4)
	tup2 = (s1, s2, s3, s4)
	statistiche = []
	for el1 in tup1:
	...
	with open('./data/clusterstats_sevscore/{}.tsv'.format(filename), 'w') as f:
		f.write('\tGroup1({})\tGroup2({})\tGroup3({})\tGroup4({})\n'.format(len(s1), len(s2), len(s3), len(s4)))
...
df1 = df[df['patient_id'].astype(str).isin(group1)].dropna()
df2 = df[df['patient_id'].astype(str).isin(group2)].dropna()
df3 = df[df['patient_id'].astype(str).isin(group3)].dropna()
df4 = df[df['patient_id'].astype(str).isin(group4)].dropna()
...
s4 = listDiff(s4a, s4b)
#s5 = listDiff(s5a, s5b)
cross_ks(s1, s2, s3, s4, 'delta_edss')
