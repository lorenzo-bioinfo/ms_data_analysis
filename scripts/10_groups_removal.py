...
for group in clean_groups:
	...
	clusterfig = sns.clustermap(rr_forced, row_linkage = cluster_row, col_cluster = False, yticklabels = True, figsize = (10, len(rr_forced)/4))
	index_row = clusterfig.dendrogram_row.reordered_ind
	plt.title('RR Clustering')
	plt.savefig('../plots/remove_clusters/rr_forced_ward_noG{}.png'.format(i), dpi = 300)
	plt.clf()
	...
	index_row = clusterfig.dendrogram_row.reordered_ind
	plt.title('RR Clustering')
	plt.savefig('../plots/remove_clusters/small_rr_forced_ward_noG{}.png'.format(i), dpi = 300)
	plt.clf()
