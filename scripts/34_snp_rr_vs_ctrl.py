...
#controls and RR patients using pearson's
#chi-squared test.
...
for entry in snp_genotp:
	samp = []
	snp_row = list(ctrl_df[entry].dropna())
	recode = []
	for gen in snp_row:
...
clean_ctrl = dict(ctrl_samp)
clean_rr = dict(rr_samp)
for snp in ctrl_samp:
	if len(ctrl_samp[snp]) == 0:
		del clean_ctrl[snp]
		del clean_rr[snp]
for snp in clean_rr:
	print(snp, clean_ctrl[snp], len(clean_ctrl[snp]))
for snp in clean_ctrl:
	print(snp, clean_rr[snp], len(clean_rr[snp]))

#and now getting a list of the remaining snps

snps_clean = list(clean_ctrl.keys())

#since I don't have expected frequencies this time,
#I will estimate them on RR patients. Then I'll compare
#the expected frequencies and test them against
#the observed frequencies in controls. In this way
#I can check wether or not they follow the same
#distribution

rr_freq = {}

for snp in snps_clean:
	tot = len(clean_rr[snp])
	if tot != 0:
		pp = clean_rr[snp].count(0)/tot
		pq = clean_rr[snp].count(1)/tot
		qq = clean_rr[snp].count(2)/tot
		rr_freq[snp] = [pp, pq, qq]
	else:
		rr_freq[snp] = [0, 0, 0]

#now I can organize the data for the chi-square tests:

chisq_data = {}
for snp in snps_clean:
	pp = clean_ctrl[snp].count(0)
	pq = clean_ctrl[snp].count(1)
	qq = clean_ctrl[snp].count(2)
	pp_exp = rr_freq[snp][0]*len(clean_ctrl[snp])
	pq_exp = rr_freq[snp][1]*len(clean_ctrl[snp])
	qq_exp = rr_freq[snp][2]*len(clean_ctrl[snp])
	chisq_data[snp] = [[pp, pq, qq], [pp_exp, pq_exp, qq_exp], len(clean_rr[snp])]

#now testing

f = open('./data/snp/tests/ctrl_vs_rr.tsv', 'w')
f.write('snp_id\tsamp_dim\tChi-stat\tp-val\n')

for snp in snps_clean:
	if((chisq_data[snp][2] != 0) and (0 not in chisq_data[snp][1])):
		stat = stats.chisquare(f_obs = chisq_data[snp][0], f_exp = chisq_data[snp][1])
		print(chisq_data[snp][0], chisq_data[snp][1])
...
df = pd.read_csv('./data/snp/tests/ctrl_vs_rr.tsv', sep = '\t')
df_corr = benjamini_hochberg(df, 'p-val')
df_corr.to_csv('./data/snp/tests/ctrl_vs_rr_bh.tsv', sep = '\t')
print(df_corr)